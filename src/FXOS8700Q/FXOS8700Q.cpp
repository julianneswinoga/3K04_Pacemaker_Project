...
FXOS8700Q_acc::FXOS8700Q_acc(PinName sda, PinName scl, int addr) : m_i2c(sda, scl), m_addr(addr) {
    ...
    // Initialize XYZ Data Configuration
    data[0] = FXOS8700Q_XYZ_DATA_CFG;
    data[1] = 0x11;  // high pass active, +/- 4g mode
    writeRegs(data,2);

    // Initialize Motion Detection Register (A_FFMT_CFG)
    data[0] = FXOS8700Q_A_FFMT_CFG;
    data[1] = 0xE0;  // latch, motion, z thresh active
    writeRegs(data, 2);
    
    // Initialize Motion Threshold Register (A_FFMT_THS)
    data[0] = FXOS8700Q_A_FFMT_THS;
    data[1] = 0x10;
    writeRegs(data, 2);

    // Initialize Motion Count (A_FFMT_COUNT)
    data[0] = FXOS8700Q_A_FFMT_COUNT;
    data[1] = 0x0A;  // 10 time steps x 5ms ODR = 50ms debounce period 
    writeRegs(data, 2);
    
   

    // // Initialize CTRL REG 2
    // data[0] = FXOS8700Q_CTRL_REG2;
    // data[1] = 0x1D;  // low power sleep, auto-sleep, low power/noise OSR
    // writeRegs(data, 2);

    // Iniitialize CTRL REG 3
    data[0] = FXOS8700Q_CTRL_REG3;
    data[1] = 0x00;  // default settings ==> interrupts are active low!
    writeRegs(data, 2);

    // Initialize CTRL REG 4
    data[0] = FXOS8700Q_CTRL_REG4;
    data[1] = 0x04;  // freefall/motion interrupt enabled
                     // could change: 0x06 to enable ffmt and a_vecm
    writeRegs(data, 2);
    // Initialize CTRL REG 5
    data[0] = FXOS8700Q_CTRL_REG5;
    data[1] = 0x04;  // ffmt on INT1 pin, a_vecm on INT2 pin
    writeRegs(data, 2);
...
uint8_t FXOS8700Q_acc::getStatus(void) {
    uint8_t stat = 0;
    readRegs( FXOS8700Q_A_FFMT_SRC, &stat, 1);
    return stat;
}

uint8_t FXOS8700Q_acc::getIntSource(void) {
    uint8_t intSrc = 0;
    readRegs ( 0x0C, &intSrc, 1);
    return intSrc;
}
